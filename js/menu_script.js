...
// global variable
const listofCategories = menuDatabase.reduce(
  function (categories, menuItem) {
...
const filterBySearchValue = function (value) {
  ...
  let regExp =
    "(\\b" + value.trim().toLowerCase().replace(/\s+/g, "|\\b") + ")";
...
const refreshModal = function (selectedMenu) {
  modalBox.querySelector(".modal-image").src =
    selectedMenu.querySelector(".menu-image").src;
  modalBox.querySelector(".menu-name").textContent =
    selectedMenu.querySelector(".menu-name").textContent;
  modalBox.querySelector(".menu-price").textContent =
    selectedMenu.querySelector(".menu-price").textContent;
  modalBox.querySelector(".modal-detail").textContent =
    selectedMenu.querySelector(".menu-detail").textContent;
};
...
const refreshReadMore = function () {
  for (let i = 0; i < displayedArticles.length; i++) {
    ...
    let descriptionHeight =
      displayedArticles[i].querySelector(".menu-description").clientHeight;
